   ...
   public static function GetBugSmybols ( $bugId, $p_newline = false )
   {
      ...
      if ( $forbiddenFlag )
      {
         if ($p_newline && !is_blank ($t_text))
            $t_text .= '<br/>' . "\n";
         $t_text .= '<a href="' . $href . '"><img border="0" width="16" height="16" src="' . plugin_file('sign_forbidden.png') . '" alt="' . $forbiddenAltText . '" title="' . $forbiddenAltText . '" /></a>';
      }
      if ( $stopFlag )
      {
         if ($p_newline && !is_blank ($t_text))
            $t_text .= '<br/>' . "\n";
         $t_text .= '<a href="' . $href . '"><img border="0" width="16" height="16" src="' . plugin_file('sign_stop.png') . '" alt="' . $stopAltText . '" title="' . $stopAltText . '" /></a>';
      }
      if ( $warningFlag )
      {
         if ($p_newline && !is_blank ($t_text))
            $t_text .= '<br/>' . "\n";
         $t_text .= '<a href="' . $href . '"><img border="0" width="16" height="16" src="' . plugin_file('sign_warning.png') . '" alt="' . $warningAltText . '" title="' . $warningAltText . '" /></a>';
      }
